...
- 本番URL: https://typingsuite.saito-kodomopg.workers.dev/
...
- **SPAルーティングとの関係**: React Router によるクライアントサイドルーティングを使うため、
  `wrangler.jsonc` の `assets.not_found_handling` を `single-page-application` に設定している。
  未設定だと `/` 以外への直リンクアクセス(例: `/practice/:taskId`)がエッジ側で404になる
  (2026-09-22、メニュー画面追加時に本番で発覚・修正)
